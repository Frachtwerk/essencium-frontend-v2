...
| Routing | TanStack Router | Type-safe, file-based, in Start integriert |
...
| Styling | Tailwind CSS | Utility-first, bereits in Nutzung |
...
| Linting | ESLint + Prettier → später Vite+ (Oxlint/Oxfmt) | Bewährt, Migration geplant |
...
- [ ] **1.1** Repo + Vite Setup (0,5 PT) — Repo erstellen, TanStack Start mit SPA-Modus konfigurieren, `dev` und `build` verifizieren
- [ ] **1.2** React Compiler + Tailwind (0,5 PT) — babel-plugin-react-compiler einbinden, Tailwind installieren und konfigurieren
- [ ] **1.3** shadcn/ui + Base UI Init (0,5 PT) — `npx shadcn init` mit Base UI, erste Components (Button, Input, Card) hinzufügen
- [ ] **1.4** Projektstruktur + Tooling (0,5 PT) — Ordnerstruktur, TypeScript-Aliase, ESLint/Prettier, Husky + lint-staged
...
- [ ] **2.1** TanStack Router Grundstruktur (0,5 PT) — File-based Routing, Root-Route, Layout-Route, Login, Dashboard, 404
...
- [ ] **4.6** Boilerplate-Dokumentation (0,5 PT) — README, Code-Kommentare, CONTRIBUTING.md, `.cursor/rules` + `CLAUDE.md`
...
- [ ] **9.4** KI-Kontextdateien (0,5 PT) — CLAUDE.md + .cursor/rules für alle Repos finalisieren
...
- [ ] TypeScript strict mode — keine `any`, keine `ts-ignore`
