...
import { MyApiTokensTable } from './my-api-tokens-table'
...
interface MyApiTokensViewProps {
  hideTitle?: boolean
}

export function MyApiTokensView({
  hideTitle = false,
}: MyApiTokensViewProps): React.ReactElement {
  const { t } = useTranslation()
  const { page, size, sort } = route.useSearch()
  const { data: tokensPage } = useFindAllApiTokens({ page, size, sort })
  ...
  function onSortingChange(updater: Updater<SortingState>): void {
    ...
    void navigate({
      search: prev => ({ ...prev, sort: serializeSort(next), page: 0 }),
    })
  }
